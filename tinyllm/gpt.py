...
class Block(nn.Module):
    ...
    def forward(self, x):
        # Layer norm swapped from Pre Norm paper
        x = x + self.attn(self.ln_1(x)) # update for unnormed residual path
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        B, T = idx.size()
        assert T <= self.config.context_length, f"cannot forward sequence of length {T}, context length is {self.config.contect_length}"
